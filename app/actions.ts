...
export async function createConsumable(formData: FormData) {
  ...
  const consumable = await prisma.consumable.create({
    data: {
      ...
      packageQuantity: optionalText(formData, "packageQuantity") ? decimal(formData, "packageQuantity") : null,
    ...
    await prisma.$transaction(async (tx) => {
      await tx.workOrder.update({
        ...
        data: {
          ...
          actionsDone: ticket.solution ?? "Ticket validado pela manutenção.",
          result: "VALIDADO",
          notes: [
  ...
  for (const row of rows) {
    ...
    await prisma.consumable.create({
      data: {
        ...
        unit: row.unidade_stock || row.unidade || "un",
        currentStock: row.stock_atual || "0",
        minimumStock: row.stock_minimo || "0",
        unitCost: (row.custo_unitario || row.unit_cost || "0").replace(",", "."),
        packageQuantity: row.quantidade_por_embalagem ? row.quantidade_por_embalagem.replace(",", ".") : null,
